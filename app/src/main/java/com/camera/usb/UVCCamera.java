...
public class UVCCamera {
    ...
    public static  int DEFAULT_PREVIEW_WIDTH = 640;//1920;640
    public static  int DEFAULT_PREVIEW_HEIGHT = 480;//1080;480
    ...
    public synchronized void open(final USBMonitor.UsbControlBlock ctrlBlock) {
        ...
        nativeExtensionOperateInitialize(mNativePtr);
    }
...
//    		mNativePtr = 0;
    ...
    public void setAudioCallback(final IFrameCallback callback) {
        if (mNativePtr != 0) {
            if (DEBUG) Log.v(TAG, "setAudioCallback");
            nativeSetAudioCallback(mNativePtr, callback);
        }
    }
	...
	//================================================================================
		 ...
		 * @param backlightComp
		 */
		public synchronized void setBacklightComp(final int backlightComp) {
			if (mNativePtr != 0) {
			   final float range = Math.abs(mBacklightCompMax - mBacklightCompMin);
			   if (range > 0)
				  nativeSetBacklightComp(mNativePtr, (int)(backlightComp / 2 * range) + mBacklightCompMin);
			}
		}
	
		/**
		 * @param backlightComp_abs
		 * @return backlightComp
		 */
		public synchronized int getBacklightComp(final int backlightComp_abs) {
		   int result = 0;
		   if (mNativePtr != 0) {
			   nativeUpdateBacklightCompLimit(mNativePtr);
			   final float range = Math.abs(mBacklightCompMax - mBacklightCompMin);
			   if (range > 0) {
				   result = (int)((backlightComp_abs - mBacklightCompMin) * 2 / range);
			   }
		   }
		   return result;
		}
		...
		public synchronized int getBacklightComp() {
			return getBacklightComp(nativeGetBacklightComp(mNativePtr));
		}
...
//================================================================================
    	...
    	if (mNativePtr != 0) {
    		if ((mControlSupports == 0) || (mProcSupports == 0)) {
    	    	...
    	    	if (DEBUG) {
					...
					Log.v(TAG, String.format("BacklightComp:min=%d,max=%d,def=%d", mBacklightCompMin, mBacklightCompMax, mBacklightCompDef));
    ...
    private static final native int nativeSetAudioCallback(final long mNativePtr, final IFrameCallback callback);
...
//**********************************************************************
    ...
    public byte[] extensionCompressRgbx(final byte[] rgbx, final int width, final int height, final int quality) {
        return nativeExtensionCompressRgbx(rgbx, width, height, quality);
    }
    ...
    private static final native byte[] nativeExtensionCompressRgbx(byte[] data, final int width, final int height, final int quality);

    private static final native int nativeExtensionCustomerValue(final long id_camera);
